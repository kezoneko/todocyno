...
@Component({
    ...
    providers: [
        { provide: MatFormFieldControl, useExisting: StatusSelectComponent }
    ]
})

export class StatusSelectComponent implements OnInit, ControlValueAccessor,
    MatFormFieldControl<Status | null>, OnDestroy, DoCheck {

    static nextId = 0;

    stateChanges = new Subject<void>();
    focused = false;
    controlType = 'app-status-select';
    id = `status-select-${StatusSelectComponent.nextId++}`;
    describedBy = '';
